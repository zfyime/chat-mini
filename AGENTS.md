...
至少填入 `OPENAI_API_KEY`。如需联网搜索，再填入 `TAVILY_API_KEY`。开发服务器默认地址为 `http://localhost:4321`。
...
- **Vercel**：使用 `pnpm build:vercel`，目标为 Vercel Serverless。
- **Docker**：使用 `docker-compose up -d --build`，服务默认运行在 `http://localhost:3000`。
...
- `docs/astro-7-upgrade-plan.md`：Astro 7 升级归档与验证记录。
