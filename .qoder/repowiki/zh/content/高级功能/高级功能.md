...
在`ChatRoot.tsx`中，流式响应被逐块读取并缓存在缓冲区中。解析器持续检查缓冲区中的`<think>`和`</think>`标签，将思考内容与常规响应内容分离，分别更新`currentAssistantThinkMessage`和`currentAssistantMessage`信号。在`MessageItem`组件中，思考内容被包裹在`<details>`元素中，默认折叠显示，用户可点击展开查看完整思考过程。
...
**图示来源**  
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L170-L211)
...
### 批量管理对话历史
系统通过`historyStore.ts`实现对话历史的本地存储和管理。`saveOrUpdateChat`函数负责保存或更新对话记录，`deleteHistory`函数提供删除功能。历史记录以时间倒序排列，最多保存`CONFIG.HISTORY_LIST_LIMIT`条记录。用户可通过点击历史按钮查看和加载过往对话。
