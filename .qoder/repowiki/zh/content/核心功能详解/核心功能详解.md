...
`ChatRoot.tsx` 是应用的核心聊天界面组件，负责协调用户输入、消息发送、流式接收与状态更新。
...
**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L15-L392)
...
- **数量限制**: 通过 `CONFIG.HISTORY_LIST_LIMIT` 限制本地存储的对话数量。
...
`chatCompletion.ts` 文件封装了与OpenAI API交互的核心逻辑。
...
**Section sources**
- [chatCompletion.ts](file://src/utils/chatCompletion.ts#L0-L72)
...
**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L15-L25)
...
**Diagram sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L260-L270)
...
### 与核心流程的集成
1. **状态管理**: `ChatRoot.tsx` 使用 `pendingAttachments` 信号存储待发送的文件。
2. **消息构造**: `handleButtonClick` 函数将 `pendingAttachments` 附加到新创建的用户消息中。
3. **API请求**: `chatCompletion.ts` 中的 `transformMessagesForAPI` 函数将附件内容嵌入API请求体。
...
- [FileUpload.tsx](file://src/components/FileUpload.tsx#L0-L137)
- [fileUtils.ts](file://src/utils/fileUtils.ts#L0-L153)
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L322-L354)
...
### 消息复制功能
`ChatRoot.tsx` 中的 `copyMessage` 函数处理消息复制逻辑：
- 接收消息内容作为参数，但目前仅用于日志记录。
- 使用 `navigator.clipboard.writeText` API 将内容写入剪贴板。
- 更新 `messageCopied` 信号以触发UI更新，显示"已复制"提示。

**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L103-L107)

### 消息删除功能
`ChatRoot.tsx` 中的 `deleteMessage` 函数处理消息删除逻辑：
- 接收消息索引作为参数，从 `messageList` 中过滤掉指定索引的消息。
- 调用 `saveOrUpdateChat` 更新本地存储中的对话历史。
- 更新 `isCurrentChatModified` 信号以标记对话已修改。

**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L90-L102)

### 与核心流程的集成
1. **组件通信**: `ChatRoot.tsx` 通过 `onDeleteMessage` 和 `onCopyMessage` 属性将函数传递给 `MessageItem.tsx`。
...
**Diagram sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L90-L107)
- [MessageItem.tsx](file://src/components/MessageItem.tsx#L121-L144)
...
**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L124-L392)
