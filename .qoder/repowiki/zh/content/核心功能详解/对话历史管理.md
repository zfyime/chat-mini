...
Prepend --> Limit["检查长度 > HISTORY_LIST_LIMIT"]
Limit --> |是| Truncate["截断至 HISTORY_LIST_LIMIT"]
Limit --> |否| Save
Save --> End([结束])
```
...
2. **历史数量限制**：通过 `CONFIG.HISTORY_LIST_LIMIT`（默认 25）限制存储的历史会话数量，防止存储膨胀
3. **内存状态管理**：使用 Solid.js Signal 实现高效的状态更新和响应式渲染
4. **按需加载**：每次启动时从 localStorage 加载数据，避免重复读取

存储策略相关配置：
- `HISTORY_LIST_LIMIT`: 25（保留最近 25 次会话）
...
#### 数据清理机制
系统自动清理机制：
- 保留最近 `HISTORY_LIST_LIMIT`（25）条记录
