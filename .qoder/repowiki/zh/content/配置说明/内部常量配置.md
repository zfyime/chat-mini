...
export const AVAILABLE_MODELS = [
  ...
  { id: 'DeepSeek-V4-Pro-0324', name: 'DeepSeek-V4-Pro' },
  { id: 'DeepSeek-R1-0528', name: 'DeepSeek-R1' },
  { id: 'grok-3', name: 'Claude-4.6-Sonnet' },
] as const
```
...
`ChatRoot.tsx` 是应用的核心生成器组件，负责管理对话状态和调用 API。
...
- **初始化状态**：组件使用 `createSignal` 创建 `chatModel` 信号，并将其初始值设置为 `CONFIG.DEFAULT_MODEL`。这确保了应用启动时有一个确定的默认模型。
  ...
  ```
- **传递设置**：`ChatRoot.tsx` 将 `temperature` 和 `chatModel` 的设置函数（`temperatureSetting` 和 `chatModelSetting`）作为 props 传递给 `SystemRoleSettings` 组件。
  ```ts
...
- **模型选择器**：组件使用 `For` 循环遍历 `AVAILABLE_MODELS` 数组，为每个模型创建一个 `<option>` 元素。`select` 元素的 `value` 属性绑定到 `chatModel()` 信号，`onChange` 事件则调用 `setChatModel` 来更新状态。
  ...
  ```
- **状态同步**：`createEffect` 用于监听 `temperature` 和 `chatModel` 信号的变化，并通过 `props.temperatureSetting` 和 `props.chatModelSetting` 将变化通知给父组件 `ChatRoot.tsx`。
  ```ts
  createEffect(() => {
...
**Section sources**
- [ChatRoot.tsx](file://src/components/ChatRoot.tsx#L29-L30)
