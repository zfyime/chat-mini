...
#### 对话与历史记录
- **`CONTEXT_WINDOW_SIZE`**: 发送给 OpenAI API 的最大上下文消息数量，控制模型的上下文长度。当前值为 `9`。
- **`HISTORY_LIST_LIMIT`**: 在本地浏览器中保留的最近历史会话数量。当会话数超过此值时，最旧的会话将被自动清除。当前值为 `25`。
...
AVAILABLE_MODELS=gpt-4o,gpt-4.1,DeepSeek-V4-Pro-0324
```
