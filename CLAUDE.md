...
- **模型动态切换**: 无需修改环境变量，直接在 UI 上选择并切换对话模型，支持 GPT-5.4、Claude-4.6-Sonnet、Gemini-3.1-Pro、GLM-5.1、DeepSeek-V4-Pro 等主流模型。
- **联网搜索 Agent**: 在 Header 通过"联网"开关启用。开启后模型可调用 Tavily 搜索互联网获取实时信息，过程在对话气泡里以可折叠面板显示。需配置 `TAVILY_API_KEY`。
...
| `TAVILY_API_KEY` | Tavily 搜索 API key。配置后才能使用 Header 的"联网"开关。 |
...
| `AGENT.TAVILY_SEARCH_DEPTH` | Tavily 搜索深度（`basic` 1 credit / `advanced` 2 credit）。 | `'basic'` |
...
项目已配置为可直接部署到 Vercel Serverless。只需将你的仓库导入 Vercel，它将自动识别 `vercel.json` 和 `package.json` 中的构建命令 `build:vercel` 并完成部署。
